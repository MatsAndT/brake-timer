...
= Introduksjon
== Prosjektbeskrivelse
Ved å bruke ATmega32-mikrokontrollere og en rekke komponenter, skulle vi ut ifra de ulike konseptene vi har lært gjennom kurset _Datamaskinarkitektur_ lage en modul/system som passer i et smarthjem.\
\
#outline(
    title: "Innhold",
    indent: auto,
...
= Metode
 En og en komponent testet og det ble laget kode for rett funksjonalitet for denne komponenten. Deretter ble disse kodeutsnittene satt sammen i en sammensatt kode for funksjonaliteten på KK-modulen og en sammensatt kode for lærer-modulen.  
...
#figure(
    ...
    caption: [
        7-segment-skjerm _(Hentet fra @Segment_display)_
    ],
    supplement: [Figur]
) <7-segment-display>
...
Lærer-modulen funker i slavemodus av KK-modulen. Lærer-modulen mottar i starten av et tidsintervall, med type intervall: «b» for pause (brake) og «l» for undervisning (lecture). Om det er en pause som blir sendt vil den sette flagget til å peke opp, og ned hvis det er undervisning. Etter type intervall så kommer hvor lenge intervallet skal varer i minutter. Denne tiden kommer som string og blir gjort om til int og ganget med 60, fordi modulen arbeider i sekunder. Stringen med denne informasjonen blir sendt fra masteren som minutter for å unngå å sende unødvendig mye data. Når det er 5 minutter igjen av en undervisning vil flagget bli heist til midten (45 grader), dette skjer uten at KK-modulen sier noe.
Lærer-modulen bruker alle innebygde timer i mikrokontrolleren, 16 bit timer1 til servoen, 8 bit timer0 for å oppdatere 7-segment-skjermen og 8 bit timer2 for å telle ned hvert sekund, for å få denne timer-en til å telle ned hvert sekund måtte vi bruke et buffer da maks tiden var 262.144ms, den ble satt til ¼ sekund og teller ned et buffer 4 ganger før den teller ned antall sekunder igjen.
